...
- Fuente: `outputs/mission/telemetry.csv` (1 paquetes).
- Altitud: 250 m → 250 m.
- Veredictos: ESTRÉS MODERADO: 1.
- Diagnósticos: AGUA EXTENSA (lago/rio): 1.
- Alertas: 0 de 1 frames.
- Sampler: FULL: 1.
- Nitidez (Laplaciano): min 212 · mediana 212 · max 212.

## 5. Evidencias generadas
- Tiles Test segmentados: 10
- Frames de evidencia de misión: 39
...
- Exportación ONNX con dynamo=False y sin dynamic_axes (requisito del conversor Sony IMX500). audit_imx500.py ahora valida de verdad contra estos requisitos y devuelve exit code != 0 si algo no cumple; antes sólo imprimía el opset y el recuento de operadores. Nota histórica: antes sólo export_onnx.py y export_b5_640.py pasaban dynamo=False; los otros cuatro eran shims que se eliminaron el 2026-09-21 (junto con evaluate_val/eval_onnx_gap/eval_int8_cpu/crf_refine), y los hints de mission_pipeline.py ya apuntan al exportador canónico. Unificado en cansat/onnxio.py + export_onnx.py. Además, audit_imx500.py ya no marca "pesos externos" por la mera presencia de un .onnx.data suelto: mira el grafo (data_location=EXTERNAL).
...
- MIL para el tipo de desastre (bolsa = evento, instancias = tiles, pooling con atención gatada) probado contra el clasificador por tile con el MISMO LOEO (24 eventos, seed 42): media ponderada de accuracy por bolsa 0.3277 vs 0.3681 del baseline por tile. NO lo supera: se documenta como técnica que no funcionó y el clasificador por tile queda. Re-run del baseline el 2026-09-21: 0.330 (varianza run-to-run ±0.04 por no-determinismo de GPU), así que MIL 0.3277 queda DENTRO de la banda del baseline: tampoco gana con la referencia más nueva. Caveat registrado: la métrica de bolsa (accuracy por bolsa, ponderada por bolsas) no es idéntica a la de tile (ponderada por tiles); 6 épocas y K=8 con eventos chicos de alta varianza. JSON: outputs/mil_loeo.json. Per-eventos donde sí gana: moore-tornado, socal-fire, woolsey-fire y palu-tsunami (acc bolsa 0.95-1.0).
...
- Calibración del clasificador de tipo (cansat/calibracion.py sobre el LOEO con las 7 probabilidades por tile, `--probs-todas`). ECE de evaluación 0.473 → 0.123 con temperature scaling (T=13.1 global; por dominio xbd 5.63, crasar y kate_pd saturan el techo del grid), NLL 3.39 → 1.66: las probabilidades crudas están muy sobre-confiadas (accuracy LOEO 0.33 con confianza ~0.9). HALLAZGO: para la política de incendio (score binario p_incendio) la temperatura es MONÓTONA, así que NO cambia la decisión: τ pasa de 0.998 a 0.690 pero el conjunto aceptado es idéntico (recall 0.260 y FPR 5.0 % en ambos). Mejora las confianzas que se muestran, no la política validada; por eso no se cablea al vuelo. Los sets adaptativos (APS) con este clasificador dan set medio = 7 (todas las clases): la cobertura se cumple trivialmente y confirman que el cuello es la accuracy del tipo, no la calibración. Evidencia: docs/benchmarks/calibracion_tipo.json. Baseline LOEO re-medido en la misma corrida: 0.330 (ver `mil-tipo-desastre`).
- Extensión con una T por clase (q_i ∝ p_i^(1/T_i), tools/calibrate_per_class.py, mismo split por eventos). NO se adopta: ECE de evaluación 0.131 vs 0.123 de la global, NLL 2.02 vs 1.66 (peor), y NO preserva el ranking de p_incendio, así que cambiaría la política fire_only_v1 validada. El ajuste por clase sobre-amortigua: casi todas las T saturan el techo del grid (20.0) y volcán queda en T=1.0 por falta de muestras en calibración (n=0). Se documenta el experimento fallido en vez de cablearlo. Evidencia: docs/benchmarks/calibracion_por_clase.json.
- Aviso de fuera-de-distribución (OOD) por frame: proxy DECLARADO, no un detector formal. Compara la mezcla de clases del terreno (divergencia Jensen-Shannon) y los índices de color (ExG y sombras, z-score) contra una referencia construida sobre LoveDA **Val** (nunca Train) con tools/build_ood_reference.py → docs/benchmarks/ood_loveda_val.json. Referencia medida: clases_prom [veg 0.362, bui 0.073, wat 0.116, bare 0.092, oth 0.357]; JS media 0.176 y umbral p95 0.380; ExG 38.0±16.0; sombras 13.5±11.9. El umbral JS es alto porque Val mezcla urbano y rural: el aviso salta con escenas muy distintas (desierto, nieve, noche) o con índices de color a >3σ. Viaja en el JSONL (ood_score/ood_flag) y la estación muestra el badge "fuera de distribución"; NO bloquea nada ni cambia decisiones: advierte que los números de Val podrían no aplicar. Si falta el JSON de referencia, el campo va en None (no se inventa).
- Perfiles de vuelo en mission_pipeline: --perfil completo (default, el validado: terreno + daño/flood/fuego/severidad con consenso) y --perfil rapido (terreno @224 + detección + estrés, sin TTA/CRF/vis ni modelos de daño). Rápido NO emite veredicto de daño: sirve para la Pi cuando el s/frame medido obligue a recortar, y así se documenta en pi/guia_pi.md. El mapeo está en PERFILES/aplicar_perfil() con tests.
- Cola de anotación para cuando existan vuelos reales (active learning mínimo): cansat/active.py puntúa cada frame con entropía del modelo de terreno (0.5) + rareza de la mezcla de clases contra la referencia de LoveDA Val (0.3) + estrés ambiental (0.2), y tools/select_tiles_for_annotation.py escribe un CSV ordenado con motivos (no copia frames). Sin labels de vuelo todavía no se re-entrena: la cola es lo que se anota primero después del primer vuelo.
...
- OpenCV 5.0.0 NO es confiable con el backend cv2.dnn del proyecto. Medido el 2026-09-21 sobre cansat_seg_terrain_v2.onnx: logits absurdos (~1e14, argmax coincide 0.01 con onnxruntime y diferencia máxima 5.8e14), mientras que OpenCV 4.13 coincide >0.99. Como la Pi Zero v1 usa cv2.dnn (no hay wheels de onnxruntime para ARMv6), requirements-flight.txt y requirements.txt fijan opencv<5 y cansat/onnxio.py avisa al cargar el backend con cv2>=5. El pin es requisito de vuelo, no estética; el test de equivalencia entre backends queda marcado slow porque necesita el ONNX de vuelo (no versionado).
- Regenerar pseudo-labels y re-destilar NO se persigue. Evidencia: la destilación (KD) ya empató con el modelo de vuelo (0.5196 vs 0.5219 mIoU) y el pipeline viejo de pseudo-labels quedó congelado en legacy/ con su defecto documentado (BN congelado, etiquetas desalineadas 1/3). Reabrirlo sin cambiar la causa raíz es exactamente el error que este repo decidió no repetir.
...
- Augmentación UAV para el daño de vuelo: ops nuevas `sombras` (parches poligonales oscurecidos) y `vibracion` (±3 px + ±0.4°, geométrica con máscara) en cansat/corrupt.py como FUENTE ÚNICA (entrenar y evaluar con la misma implementación), subset AUG_UAV = motion_blur/escala/sombras/ vibracion, hook --aug-uav en train_damage_v3.py (default apagado). Baseline medido (dano2, RescueNet val, n=60): limpio IoU bin 0.374; motion_blur -0.021, escala -0.026, sombras -0.001, vibracion -0.006. Fine-tune desde best_damage3_bal.pth con aug (1 época, lr 1e-4): limpio 0.265 (-0.109), motion_blur -0.034, escala +0.000. NO SE ADOPTA: degrada el limpio. Causas probables: mismatch de receta (init de train_damage_v2, fine-tune con v3), 1 época insuficiente y aug agresiva (escala 0.45-0.60 con p=0.35×4 ops). El A/B correcto (mismo init, misma receta ± aug, schedule largo) queda para V11. Evidencia: docs/benchmarks/aug_uav_dano.json.
- Firmware emisor del contrato LB135 v2 para Heltec WiFi LoRa 32 V3 (firmware/heltec_lb135_uart/, PlatformIO): 1 paquete/s con perfil de descenso simulado DECLARADO (no son sensores reales) y checksum NMEA. El formato se validó contra cansat/protocol.py (parse + checksum + fix). Sale por USB-serie: en la PC se lee como COMx y en la Pi como /dev/ttyACM0, sin pelear con pines GPIO. Cadena probada en PC: sim_uart → uart_listener --stdin → uart_state.json (12/12 paquetes). PENDIENTE: flashear la placa, probar el listener contra el Heltec real y el cableado GPIO directo (TX→GPIO15); ver pi/guia_pi.md §8.
- Explicabilidad del daño integrada a la estación (2026-09-22): endpoint GET /api/gradcam (src, modelo=dano2|dano, clase opcional) que invoca tools/gradcam.py por subproceso con el venv de vuelo (la estación sigue stdlib-only), cachea el PNG en outputs/gradcam/ y lo sirve por /img/. El Detalle del frame tiene botón "explicar (Grad-CAM)" con selector de modelo (vuelo/xBD) y overlay mostrar/ocultar; el resultado vale solo para su frame y la petición se aborta al navegar. Primera generación ~13 s en CPU, después cache. Sin torch/checkpoint degrada con error claro. Evidencia: docs/evidencia/10_gradcam.png.
