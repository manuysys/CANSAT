...
- Consulta Terrestre en la estación: consultas espaciales simbólicas sobre las máscaras de clase por frame. El post-vuelo ahora las persiste en entrega/masks/ (PNG gris de índices, 255 = sin dato; se apaga con --no-masks y suma el bucket del contrato v3). Motor: cansat/consultas.py (plantillas en español → área, conteo con buffer métrico, fracción de longitud por esqueleto, distancia y personas de la telemetría). NO hay LLM como respondedor: lo que no mapea devuelve "consulta no soportada" con sugerencias. El servidor de la estación lo invoca por subproceso (tools/consulta.py), así sigue siendo stdlib-only. Georreferenciado declarado APROXIMADO: cuadrado de lado sqrt(area_m2) centrado en (lat, lon), norte arriba, sin heading (la telemetría no trae IMU). Las personas se cuentan por frame (no hay posición por detección) y la longitud es ±10 % por esqueletización. Las consultas de vías dependen de train_road_specialist.py: si su IoU de vía en FloodNet val no supera 0.50, quedan "no soportadas" y se documenta la técnica como no funcionó (no se integra un modelo peor al pipeline).
...
- El benchmark del parser de la consulta (tools/bench_parser_earthvqa.py) mide cobertura de plantillas por tipo de pregunta y precisión de mapeo sobre una muestra etiquetada a mano; NO mide exactitud de respuesta VQA. El QA real de EarthVQA está GATED en Hugging Face: la descarga devuelve 403 (hay que aceptar las condiciones con la cuenta). No se inventa la métrica: queda pendiente y documentada. El parser ahora acepta sinónimos en inglés (building/road/water/flooded/…), además del español, con tests.
